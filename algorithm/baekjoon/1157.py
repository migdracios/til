# 최빈값 알고리즘

'''
- 문제 : 
알파벳 대소문자로 된 단어가 주어지면, 이 단어에서 가장 많이 사용된 알파벳이 무엇인지 알아내는 프로그램을 작성하시오. 단, 대문자와 소문자를 구분하지 않는다.
- 입력 : 첫째 줄에 알파벳 대소문자로 이루어진 단어가 주어진다. 주어지는 단어의 길이는 1,000,000을 넘지 않는다.
- 출력 : 첫째 줄에 이 단어에서 가장 많이 사용된 알파벳을 대문자로 출력한다. 단, 가장 많이 사용된 알파벳이 여러 개 존재하는 경우에는 ?를 출력한다.
'''

'''
알고리즘 설계
1. 입력 값을 대문자로 변환
2. 알파벳 길이 26의 리스트 생성
3. 입력 값을 반복하면서 해당 알파벳의 위치 값에 +1
4. 리스트 중에 가장 큰 값을 찾아내어 알파벳 대문자로 출력
'''

target_text = input()
target_text = target_text.upper()
alphabet_list = [0] *26
max_count = 0
max_alpha = target_text[0]
for alphabet in target_text:
    alphabet = ord(alphabet) - ord("A")
    alphabet_list[alphabet] += 1
    if alphabet_list[alphabet] > max_count:
        max_count = alphabet_list[alphabet]
        max_alpha = alphabet
print(chr(max_alpha+ord("A")))


