...
def num_croatia(text, count):
    ...
    5. 문자열의 공백을 replace으로 제거한 뒤 len 메서드의 길이만큼 카운트에 추가, 카운트 return
    '''
...
3. 같은 문자열이 여러 개 있는 경우 한 번이 아니라 여러 개를 삭제해버림 -> replace 세 번째 인자로 횟수를 1회로 고정
4. 문자열이 더 이상 남아있지 않은 경우 리턴 카운트 실행이 되지 않음 -> 아래쪽 count 더해주기 로직에서 모두 해결 가능하기에 삭제
'''
